...
Next week, we will talk more about making more attractive plots that would go into final reports. 
...
```{r}
data(nepali)
nepali <- subset(nepali, !duplicated(id),
                 select = c("id", "sex", "wt", "ht", "age"))
nepali$id <- factor(nepali$id)
nepali$sex <- factor(nepali$sex, levels = c(1, 2), 
...
```{r, fig.width = 4, fig.height = 3}
hist(nepali$ht, main = "Height of children", 
     xlab = "Height (cm)", xlim = c(0, 120), col = "blue")
```

## `hist()` example

`hist` also has its own special function, `breaks`. You can use this to change the number of bins that are used to make the histogram:
...
plot(nepali$ht, nepali$wt, 
     ...
     cex = 0.5, col = "blue", main = "Weight versus Height")
```

## `pairs()` example

You can use the `pairs()` function to plot all pairs of scatterplots for several variables. 
...
boxplot(nepali$ht, ylab = "Height (cm)")
```
...
boxplot(nepali$ht ~ nepali$sex, ylab = "Height (cm)",
        main = "Height boxplots by sex")
```
...
with(nepali, plot(ht, wt, cex = 0.5,
                  xlab = "Height (cm)", ylab = "Weight (kg)"))
abline(mod_a, col = "blue")
```
...
```{r}
nepali$wt_over_13 <- nepali$wt > 13
head(nepali$wt_over_13)
```
