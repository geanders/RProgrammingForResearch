...
To find out all of the datasets available within a certain package, run `data` with a `package` argument:
...
Notice that you first specify the dataframe with the data you want to plot and then you might specify either **mappings** or constant values for some or all of the aesthetics. 
...
*Note*: Any of these aesthetics could also be given a constant value, instead of mapped to an element of the data. For example, all the points could be red, instead of showing gender. 
...
Which aesthetics you need to specify depend on which geoms (more on those in a second) you're adding to the plot. \medskip 

You can find out the aesthetics you can use for a geom in the "Aesthetics" section of the geom's help file. \medskip 

Required aesthetics are in bold and optional ones are not. 
...
```{r, fig.width = 3.5, fig.height = 3, eval = FALSE}
...
```{r, fig.width = 8, fig.height = 3, message = FALSE, eval = FALSE}
ggplot(worldcup, aes(x = Time)) + 
        geom_histogram(binwidth = 10) + 
        geom_vline(aes(xintercept = 90 * 0:6),
...
nepali <- nepali %>%
  ...
  mutate(id = factor(id, levels = c(1, 2), labels = c("Male", "Female")),
         sex = factor(sex)) %>%
