...
that use these operators to conduct the test. The output is a  
...
You can use *logical vectors*, created with these expressions, for a lot data exploration 
tasks. We'll review them and add some more details this week.
...
```{r}
length(distinct_nepali$sex)
length(distinct_nepali$sex == "Male")
```
...
```{r}
head(distinct_nepali$sex)
head(distinct_nepali$sex == "Male")
```
...
```{r}
distinct_nepali <- distinct_nepali %>% 
...
each child's first-measured height is over 100 centimeters:
...
You can "flip" a logical vector (i.e., change every `TRUE` to `FALSE` and
vice-versa) using the bang operator (`!`):
...
with the `filter` function to pull out just the rows where `is_male` is `TRUE`:
...
```{r}
head(distinct_nepali$is_male)
```

```{r}
head(as.numeric(distinct_nepali$is_male))
```
...
We've looked at how to subset, filter, arrange, and add to a dataframe. Next
we'll look at how to summarize a dataframe.

We'll start by looking at some simple statistics functions from base R, and then
we'll look at how some of those functions can be used with the `summarize`
function from the `dplyr` package to quickly get interesting summaries of data.
...
statistic. If there are missing values in the vector, you'll need to add an
option to say what to do when them (e.g., `na.rm` or `use="complete.obs"`---see
help files).

```{r}
mean(distinct_nepali$wt, na.rm = TRUE)
min(distinct_nepali$wt, na.rm = TRUE)
```
...
```{r}
cor(distinct_nepali$wt, 
    distinct_nepali$ht, 
    use = "complete.obs")
cor(distinct_nepali[ , c("wt", "ht", "age")], 
    use = "complete.obs")
```
...
```{r}
summary(distinct_nepali$wt)
summary(distinct_nepali$sex)
```

## `summary()`: A bit of OOP

```{r}
distinct_nepali %>% summary()
distinct_nepali %>% pluck("ht") %>% summary()
distinct_nepali %>% pluck("sex") %>% summary()
```
