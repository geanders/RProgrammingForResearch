...
Today, we'll talk about using functions from the `dplyr` pakcage, as well as 
a few packages for working with specific types of data (`stringr` for character 
strings, `forcats` for factors, and `lubridate` for dates),
which are all part of the "tidyverse", like the `readr` package.
...
To rename these columns, use the `rename` function from the `dplyr` function. 
...
Here's a basic example of using `select` to extract based on column names:
...
```{r}
hootie_shows <- select(slice(.data = daily_show, 64:65), 
                       date, guest_name)
...
For example, to reorder the `daily_show` data by job category, you can run:

```{r}
daily_show <- arrange(daily_show, desc(category))
head(daily_show, 3)
```
