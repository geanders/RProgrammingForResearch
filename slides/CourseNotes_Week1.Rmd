...
Double-click on the icon for "RStudio" on you computer.
...
The **console** lets you "talk" to R. This is where you type an **expression** at the
**prompt** (the caret symbol, ">"). You press the "Return" key to submit this message to R.
...
## R expressions and function calls
...
```{r}
print(x = "Hello world", print = TRUE)
print(x = "Hello world")
```
...
[Navigating the helpfile for `mean`]

## Objects, object names, and assignments
...
You can think of an **object name** as kind of like an acronym. \medskip

> "Here, we investigate particulate matter with a diameter less than 2.5
micrometers ($PM_{2.5}$). Many large urban centers in developing countries
experience multi-day episodes of extremely high $PM_{2.5}$."

By assigning an object a name, you can quickly reference it later, rather than
having to re-define (or re-create) the object every time you need to use it.
...
[name I want to use to refer to the object] <- [object I want to assign a name to]
```
...
However, if I assign it to an object, I can "refer" to that object in a later function 
call. 
...
[Example of Environment pane]



## Naming objects

There are some rules for the names you can use for an object:
...
The console can be great for quick functions to explore the data. However, for
most data analysis work you'll want to use a script, so you can save all the
code you used for the analysis. This improves the *reproducibility* of your
analysis.
...
did again later. You can also share the script for someone else to use, or run
...
- I recommend that you make an "R" folder in all of the R projects that you
create and save all your script files in that folder.
...
get in the habit of writing your R code in reproducible formarts (R scripts,
RMarkdown documents)

## In-Course Exercises

We'll take a break now to finish this week's in-course exercise. You can find
all the instructions on the online coursebook
(https://geanders.github.io/RProgrammingForResearch/r-preliminaries.html).

Get together with your group members and do Sections 1.8.4 through 1.8.8 of the
...
The most popular place from which to get packages is currently CRAN. You can
install packages from CRAN using R code.

## CRAN 

The Comprehensive R Archive Network (CRAN) is the primary source of R packages.
Each of the packages available on CRAN has a unique name and has passed some
broad checks.
...
function, which by defaults installs packages available on CRAN.
...
case, you can open the vignette using a shorter call:
...
One class of objects can be a subtype of a more general object class. We'll 
see an example of this today.

Today, we'll look at two key object classes for working with data: 
...
1. How to make a new object with that class
2. How to extract values from objects with that class

In later classes, we'll spend a lot of time learning how to do other things
with objects from these two classes.
...
The two most common types of objects for data in R that we will use in this
class are **vectors** (1D) and **dataframes** (2D).

## Vectors

- A vector is a string of values. 
- All values must be of the same *class* (i.e., all numbers, all characters, all
dates)
...
Vectors can have several different classes, including numeric, character,
factor, and date.

Typically, when you use `c`, you'll be creating a numeric or character vector.
More complex classes (like factors and dates) require a bit more work to create
"from scratch".
...
A dataframe is one or more vectors of the same length stuck together
side-by-side. It is the closest R has to what you'd get with an Excel
spreadsheet.

\medskip

You can create dataframes using the `data_frame` function from the `tibble`
package. However, most often you will create a dataframe by reading in data from
a file using something like `read_csv` from the `readr` package---most datasets
...
For example, to get summaries of the first two columns of the Daily Show data,
you can run:
...
We'll take a break now to get started on this week's in-course exercise. You can
find all the instructions on the online coursebook
(https://geanders.github.io/RProgrammingForResearch/r-preliminaries.html).

Get together with your group members and do Sections 1.8.1 through 1.8.4 of the
In-Course Exercise for Week #1 ("R Preliminaries").
