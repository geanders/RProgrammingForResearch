...
- Learn about bioconductor at [bioconductor.org](https://bioconductor.org/packages/devel/bioc/vignettes/microbiome/inst/doc/vignette.html). Bioconductor provrides opensource software for bioinfomatics. The Bioconbductor provides an R package called `microbiome` that includes tools for exploring and analysing microbiome profiling data. We want to explore a dataset on genus-level microbiota profiling (atlas1006). Navigate this page to figure out how you can get this example raw data (hint: you'll need to follow the installation instructions on the bioconductor website, which are a little different compared to a regualr R package). Create an object in your workspace containing the atlas1006 data. How is the data structured in the object you just created?
