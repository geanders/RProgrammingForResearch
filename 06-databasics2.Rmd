...
- Learn about bioconductor at [bioconductor.org](https://bioconductor.org/packages/devel/bioc/vignettes/microbiome/inst/doc/vignette.html). Bioconductor provides opensource software for bioinfomatics. Bioconductor provides an R package called `microbiome` that includes tools for exploring and analysing microbiome profiling data. We want to explore a dataset on genus-level microbiota profiling (atlas1006). Navigate this page to figure out how you can get this example raw data (hint: you'll need to follow the installation instructions on the bioconductor website, which are a little different compared to a regualr R package).
- Create an object in your workspace containing the `atlas1006` data. How is the data structured in the object you just created?
- On the [microbiome page](https://bioconductor.org/packages/devel/bioc/vignettes/microbiome/inst/doc/vignette.html), find the documentation describing the `atlas1006` data. Look through this documentation to figure out what information is included in the data.
- Use the `str` function to take a look at the structure of the `atlas1006` data. How many slots does the object contain?
- Extract ...
...
- Discuss which of the variables in this dataset could be used to merge the extracted with ...
...
diversity <- as_tibble(sample_data(atlas1006)) %>%
  dplyr::select(gender, nationality, diversity, bmi_group)


diversity %>% 
  filter(!is.na(nationality) & !is.na(gender) & !is.na(bmi_group)) %>%
  group_by(nationality, gender, bmi_group) %>%
  summarize(mean_diveristy = mean(diversity, na.rm = TRUE),
            sd_diveristy = sd(diversity, na.rm = TRUE)) %>%
  ggplot(aes(x = nationality, y = mean_diveristy, color = gender)) +
  geom_point(alpha = 0.25, size = 4) +
  xlab("Nationality") +
  ylab("Mean Diversity Score") +
...
diversity <- as_tibble(sample_data(atlas1006)) %>%
  dplyr::select(sex, nationality, diversity, bmi_group)


diversity %>% 
  filter(!is.na(nationality) & !is.na(sex) & !is.na(bmi_group)) %>%
  group_by(nationality, sex, bmi_group) %>%
  summarize(mean_diveristy = mean(diversity, na.rm = TRUE),
            sd_diveristy = sd(diversity, na.rm = TRUE)) %>%
  ggplot(aes(x = nationality, y = mean_diveristy, color = sex)) +
  geom_point(alpha = 0.25, size = 4) +
  xlab("Nationality") +
  ylab("Mean Diversity Score") +
  facet_wrap(~ bmi_group) +
