...
The `broom` packge contains tools for converting statistical objects into nice tidy data frames. The tools in the `broom` package make it easy to process statistical results in R using the tools of the `tidyverse`.

### broom::tidy

The `tidy()` function returns a data frame with informaion on the fitted model terms. For example, when applied to one of our linear models we get:
...
The `glance()` functions returnsa one row summary of a fitted model object: For example:
