...
t_tests <- filtered_data %>%
  mutate(t_test = map(data, ~t.test(abundance ~ sex, data = .x)))
```
...
flights %>% 
  ...
  group_by(carrier) %>% 
  nest() %>% 
  mutate(glm_result = map(data, ~ glm(late_dep ~ hour, 
                                      data = .x, family = binomial(link = "logit")))) %>% 
  mutate(glm_tidy = map(glm_result, ~ tidy(.x))) %>% 
...
word_list %>% 
  ...
  separate(word, into = c("first_word", "second_word"),          # Note that you can use `separate`
           sep = 4, remove = FALSE) %>%                          #   with a number to use position
  ...
  unite("new_word", c("first_word", "second_word"), sep = " ")   # You can use `unite` to put the
                                                                 #   two new words in a phrase in
                                                                 #   one column.
```
