...
library(ggplot2)
ggplot(peerj32_tibble, aes(x = subject, y = abundance, color = gender)) +
  geom_point() +
  theme_bw() +
...
We can then group the data by `phylum` and `group` and use the `nest` function to create a new dataframe containing the nested data:
...
We can then use the `map` function from the `purrr` package to apply functions to each nested dataframe. Let's start by counting the rows in each nested dataframe and filtering out dataframe with less than 25 rows:

We can then apply functions such as the `t.test` function to each nested dataframe.

```{r}
library(purrr)

filtered_data <- nested_data %>%
  mutate(n_rows = map(data, ~nrow(.x))) %>%
  filter(n_rows > 25)
```
...
t_tests <- filtered_data %>%
  mutate(t_test = map(data, ~t.test(abundance ~ gender, data = .x)))
t_tests
```

The resulting dataframe contains a new column containing the model objects. Just as we mapped the `t.test` function, we can map the `tidying` functions from the `broom` package to extract the information we need from the nested model objects.

```{r}
summary <- t_tests %>%
  mutate(summary = map(t_test, broom::glance))
```

The final step is to return a tidy dataframe, we can do this usiing the `unnest` function. Note: its always wise to `ungroup` after you are done operating on your grouped variables - you may run into problems if you forget your dataframe is grouped later on!
...
We can then plot the results for the "LGG" (non-placebo) group.
...
```{r}
library(stringr)
student_list <- tibble(name = c("Aeriel", "Rebecca", "Grant",
                                    "Amy", "Jessy", "Alyssa",
