...
# Use `read_delim` to read this file. You need to specify what the delimiter 
# is with the `delim` argument. `\t` stands for "Tab".
ld_genetics <- read_delim("data/ld_genetics.txt", delim = "\t")
summary(ld_genetics)

# Use `read_delim` to read this file. You need to specify what the delimiter 
# is with the `delim` argument. `\t` stands for "Tab". Because the first line
# of the file is *not* the column names, you need to specify what the column
# names should be with the `col_names` parameter. 
ca_measles <- read_delim("data/measles_data/02-09-2015.txt",
                         col_names = c("city", "count"), delim = "\t")
head(ca_measles)
