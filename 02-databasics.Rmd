...
Download the whole directory for this week from Github. Put the directory for Week 2 as a subdirectory in your directory for this class. To do that, go the the [GitHub page for the course](https://github.com/geanders/RCourseFall2015) and, in the bottom right, choose "Download Zip". Then you can move the directory for this Week into your directory for this course and throw away rest of the directory. 

Look through the structure of this week's directory. What files are in the directory? What subdirectories? Sketch out the structure of this directory.

### R script file

Create a new R script file to put all the code you use for this exercise. Save it in the directory for this week. 

### Using relative and absolute file names

First, I'd like you to try using `setwd()` to move around the directories on your computer. I want you to try to open the same file from different working directories, to get practice using absolute and relative file path names. 

Start in the working directory `Week2_Aug31/example_data/`. For me, the absolute path to that is `/Users/brookeanderson/Desktop/RCourseFall2015/Week2_Aug31/example_data`, so I can run:

```{r eval = FALSE}
my_dir <- paste0("/Users/brookeanderson/Desktop/RCourseFall2015/",
                 "Week2_Aug31/example_data")
setwd(my_dir)
getwd()
```

(The absolute pathname will be different for each person, based on the directories they have on their computer and where they saved this particular directory.)

Now you can check to make sure you have all the files you want:

```{r eval = FALSE}
setwd(my_dir) # Note: I have to re-set here for knitr. You won't have to.
list.files()
```

Try the following tasks: 

* Read in the ebola data in `country_timeseries.csv`. Save is as the R object `ebola`. How many rows and columns does it have? What are the names of the columns?
* Now try moving up one directory from your working directory, into the directory `Week2_Aug31\`. What happens if you use the same code as before to read in the file? What do you need to change to be able to read the file in from here? Try to read in the data from here using:
    + A relative pathname
    + An absolute pathname
* Now move down into the subdirectory of `example_data` called `measles_data`. Try to read in the Ebola data from here using: 
    + A relative pathname
    + An absolute pathname
* Which method was the same, regardless of your current working directory? Which methods was different?
...
ebola <- read.csv("example_data/country_timeseries.csv",
                  header = TRUE) 
ebola[1:5, 1:5]

## Get the file using the absolute pathname
abs_path <- paste0("/Users/brookeanderson/Desktop/",
                   "RCourseFall2015/Week2_Aug31/",
                   "example_data/country_timeseries.csv")
abs_path
ebola <- read.csv(abs_path,
                  header = TRUE) 
ebola[1:5, 1:5]

## Move into the measles_data subdirectory
setwd("example_data/measles_data")
getwd()

## Get the file using the relative pathname
ebola <- read.csv("../country_timeseries.csv",
                  header = TRUE) 
ebola[1:5, 1:5]
...
- On the [GitHub page for this week's data](https://github.com/geanders/RCourseFall2015/tree/master/Week2_Aug31/example_data), I've included some information about where I got all the example data. Find out some more about the Ebola dataset by following the link I've included to Caitlin Rivers' GitHub repository. Who is Caitlin Rivers? How did she put this dataset together?
- When you `list.files()` inside the `example_data` directory, almost everything listed has a file extension, like `.csv`, `.xls`, `.sas7bdat`. One thing does not. Which one? Why does this listing not have a file extension?

### Reading in different types of files

First, make sure you're in the `example_data/` directory again:
...
Now I'd like you to try reading in data from a variety of types of file formats. 
...
- Check out the file `measles_data/02-09-2015.txt`. What type of flat file do you think it is? Stay in the `example_data` directory and use a relative pathname to read it into the R object `ca_measles`. Use the `col.names` option to name the columns "city" and "count". What would the default column names be if you didn't use this option?
...
Example R code (note: for some of these, you may get messages I haven't printed out here):

```{r, message = FALSE, eval = FALSE}
setwd(my_dir) # Note: I have to re-set here for knitr. You won't have to.

ld_genetics <- read.delim("ld_genetics.txt", header = TRUE)
summary(ld_genetics)
...
- Is there a way to read the `ld_genetics.txt` file in using `read.table()` and specific options? If so, try to read the data in using that function.
...
ld_genetics <- read.table("ld_genetics.txt", header = TRUE,
                          sep = "\t")
ld_genetics[1:5, 1:5]
```
...
ebola_liberia <- subset(ebola, 
                        select = c("Date", "Cases_Liberia",
                                   "Deaths_Liberia"))
head(ebola_liberia)
...
ebola_liberia$date <- as.Date(ebola_liberia$date,
                              format = c("%m/%d/%Y"))
...
Here's an example to give you a feel for why it's worth learning all these different things about directories and `list.files()` and different pathnames, etc. 
...
We'll talk more later about using loops and functions to make your programming more efficient, but for right now just look through this code and see if you can get a feel for how it's working:

```{r, fig.width=10, fig.height=4, eval = FALSE}
setwd(my_dir) # Note: I have to re-set here for knitr. You won't have to.

## Create a vector of all the file names in the `measles_data` subdirectory
measles_files <- list.files("measles_data")
measles_files
...
## I want to read.
file_name <- paste0("measles_data/", measles_files[i])
file_name
...
library(ggplot2)
ggplot(subset(ca_measles, city %in% c("LOS ANGELES",
                                      "SAN DIEGO",
                                      "ORANGE")),
       aes(x = date, y = count)) + 
        geom_line()  + 
