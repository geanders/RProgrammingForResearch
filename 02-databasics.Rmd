...
Your computer organizes files through a collection of directories. Chances are, you are fairly used to working with these in your daily life already (although you may call them "folders" rather than "directories"). For example, you've probably created new directories to use to store data files and Word documents for a specific project. 

Here's a figure that illustrates the file directory structure on my computer. (Note that I have omitted many, many additional files and directories-- this just shows an example of a few directories and files and how they are structured together). Directories are shown in blue and files in green. 

![]("figures/FileDirectoryStructure.png")

You can notice a few interesting things from this figure. First, you might notice the structure includes a few of the directories that you use a lot on your own computer, like `Desktop`, `Documents`, and `Downloads`. `Desktop` is the directory that contains all the directories and files that show up on your computer's Desktop. `Downloads` is the default place where downloaded files are saved. 

Next, the directory at the very top is the computer's root directory, `\`. For a PC, the root directory might something like `C:`; for Unix and Macs, it's usually `\`. 

Finally, if you look closely, you'll notice that it's possible to have different files in different locations of the directory structure with the same file name. For example, in the figure, there are files names `heat_mort.csv` in both the `CourseText` directory and in the `example_data` directory. These are two different files with completely different contents, but they can have the same name as long as they're in different directories. This fact-- that you can have files with the same name in different places-- how useful it is that R requires you to give very clear directions to describe exactly which file you want R to read in, if you aren't reading in something in your current working directory.
...
Anytime you run R, it's always doing its work based in some working directory, which will be one of the directories somewhere within your computer's directory structure. At any time, you can figure out what directory R is working in by running the command `getwd()`. For example, my R session is currently running in the following directory:

```{r}
getwd()
```

This means that, for my current R session, R is working in the `CourseText` subdirectory of my `RCourseFall2015` directory, which is a subdirectory itself of my `Desktop`, which is a subdirectory of my home directory (`\Users\brookeanderson`).
...
Once you get a picture of how your directories and files are organized, you can use pathnames, either absolute or relative, to move around the directories, set a different working directory, and read in files from different directories than your current working directory. 

There are two ways to specify pathnames (i.e., the directions for getting to a directory or file): absolute pathnames and relative pathnames. Absolute pathnames are a bit more straightforward, because they don't depend on your current working directory. However, they're also a lot longer to write, and they're much less convenient if you'll be sharing some of your code with other people who might run it on their own computers. I'll explain this second point a bit more a little later in this section. 
...
You can use this absolute pathname to read this file in using `read.csv`, and this absolute pathname will *always* work, regardless of your current working directory, because it gives directions from the root, so it will always be clear to R exactly what file you're talking about. Here's the code to use to read that file in using its absolute pathname:

```
heat_mort <- read.csv("\Users\brookeanderson\Desktop\RCourseFall2015\CourseText\heat_mort.csv")
```

The **relative pathname**, on the other hand, gives R the directions for how to get to a directory or file *from the current working directory*. If the file or directory you're looking for is pretty close to your current working directory in your directory structure, then this will be a (possibly much) shorter way to tell R how to get there. However, the relative pathname depends on what your current working directory is, and the relative pathname that works perfectly when you're working in one directory will not work at all once you move into a different working directory. 

As an example of a relative pathname, say you're working in the directory `RCourseFall2015` and you want to read in the `heat_mort.csv` file in the `CourseText` directory. To get from `RCourseFall2015` to that file, you'd need to look in the subdirectory `CourseText`, where you could find `heat_mort.csv`. Therefore, the relative pathname from your working directory would be:

```
"CourseText/heat_mort.csv"
```

You can use this relative pathname instead of the absolute pathname to help R find and read in the file:

```
heat_mort <- read.csv("CourseText/heat_mort.csv")
```

While this is much shorter, it is important to remember that if you changed your working directory (for example, if you used `setwd("CourseText")` to move into the `CourseText` directory), this relative pathname would no longer work. 
...
---------         | ---------
`~`               | Your home directory
`..`              | One directory up from your current directory
`../..`           | Two directories up from your current directory
...
### Pathnames: Relative versus absolute
