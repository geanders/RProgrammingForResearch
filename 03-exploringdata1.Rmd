...
- Plot scatterplots of different combinations of numeric variables (e.g., `Time` vs. `Shots`). Try doing this using the `geom_point()` geom from `ggplot2`. Try using different constant or mapped values with the `color` aesthetic. 
...
- Try customizing the number of bins used for the histogram plotted in the previous step.
...
- In the previous section of the group exercise, you should have identified that the four top teams in this tournament were Spain, the Netherlands, Germany, and Uruguay. Create a histogram using data only from these four citis for the amount of time each player played. Use the color aesthetic of the histogram to show team. 
