...
hist(worldcup$Time, main = "Time")
hist(worldcup$Time, main = "Time", breaks = 50)
hist(worldcup$Time, main = "Time", breaks = seq(from = 0, to = 600, by = 100))
```
...
```{r, fig.width = 3, fig.height = 3}
plot(worldcup$Time, worldcup$Passes)
plot(worldcup$Time, worldcup$Passes, cex = 0.5)
## You can use the `with` function to save some time typing out the 
## dataframe name
with(worldcup, plot(Time, Passes))
```

Use `pairs` to plot scatterplots of all combinations of several numeric variables.

```{r}
pairs(worldcup[ , c("Time", "Shots", "Passes", "Tackles", "Saves")],
      cex = 0.5)
```

To create a boxplot of `Shots`, you can use the `boxplot()` command:

```{r, fig.width=3, fig.height=3}
boxplot(worldcup$Shots)
```

If you want to do separate boxplots of `Shots` by `Position`, you can run:

```{r, fig.width=6, fig.height=3}
boxplot(worldcup$Shots ~ worldcup$Position)
```
...
```{r}
best_four <- c("Spain", "Netherlands", "Germany", "Uruguay")
top_teams <- subset(worldcup,
                    Team %in% best_four)
```
...
```{r}
top_teams$Team <- as.factor(as.character(top_teams$Team))
```

```{r, fig.width = 6, fig.height = 3}
boxplot(top_teams$Shots ~ top_teams$Team, main = "Shots")
boxplot(top_teams$Saves ~ top_teams$Team, main = "Saves")
```
...
subset(worldcup, Time == max(worldcup$Time))
```
...
```{r}
teams_most_time <- worldcup[worldcup$Time == max(worldcup$Time) , "Team"]
teams_most_time
```
...
```{r}
most_time <- rownames(subset(worldcup, Time == max(worldcup$Time)))
most_time
```
...
```{r}
plot(chic$temp, chic$dptp, cex = 0.25, col = "gray")
abline(mod_1)
```
...
```{r, fig.height = 3, fig.width = 6}
boxplot(chic$pm10 ~ chic$dow, ylim = c(0, 100))
```

Create a subset called `summer` with just the summer days:

```{r}
summer <- subset(chic, month %in% 6:8)
```
