...
You can add some elements to the histogram now to customize it a bit. For example, you can add a figure title (`ggtitle`) and clearer labels for the x-axis (`xlab`). You can also change the range of values shown by the x-axis (`xlim`).

```{r, fig.width = 4, fig.height = 3, fig.align = "center", message = FALSE, warning = FALSE}
ggplot(nepali, aes(x = ht)) + 
...
The geom `geom_histogram` also has its own special argument, `bins`. You can use this argument to change the number of bins that are used to make the histogram:

```{r, fig.width = 4, fig.height = 3, fig.align = "center", warning = FALSE, message = FALSE}
ggplot(nepali, aes(x = ht)) + 
  geom_histogram(fill = "lightblue", color = "black",
                 bins = 40) 
```
...
A scatterplot shows how one variable changes as another changes. You can use the `geom_point` geom to create a scatterplot. For example, to create a scatterplot of height versus age for the Nepali data: 
